 ...
 * emitLog()
 * ------------
 * Stuurt een logbericht naar alle clients én bewaart het in de in-memory logBuffer.
 * Hierdoor blijven logs bewaard bij reconnects en kunnen admins bij join alle vorige logs zien.
 */
export function emitLog(
  ...
  // Voeg toe aan in-memory log buffer
  logBuffer.unshift(entry);
  if (logBuffer.length > LOG_MAX) {
    logBuffer.pop();
  }

  // Stuur log naar alle verbonden admins
  io.emit("log", entry);
}
    ...
    try {
      ...
      switch (action) {
        ...
        case "addToQueue":
          ...
          emitLog({
            type: "join",
            message: `@${username} toegevoegd aan wachtrij`,
          });
          break;
...
app.post("/api/admin/:action", requireAdmin, async (_req, res) => {
  res.json({
    success: true,
    message: "REST endpoint klaar – gebruik socket voor live",
  });
});
...
initDB().then(async () => {
  server.listen(4000, () => {
    console.log(
      "BATTLEBOX 5-ENGINE + ADMIN DASHBOARD LIVE → http://localhost:4000"
    );
  ...
  conn.on("chat", async (data: any) => {
    ...
    if (
      ...
      msg.toLowerCase().startsWith("!adm voegrij @")
    ) {
      const target = msg.split("@")[1]?.split(" ")[0];
      if (target) {
        const res = await pool.query(
          "SELECT tiktok_id FROM users WHERE username ILIKE $1",
          [`%@${target}`]
        );
        if (res.rows[0]) {
